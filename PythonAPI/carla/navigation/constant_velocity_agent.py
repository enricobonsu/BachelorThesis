...
class ConstantVelocityAgent(BasicAgent):
    ...
    def run_step(self):
        """
        Function used for generating expert behaviour
        """
        ...
        # trafficCodeReturn 1 means the traffic light is observed red
        max_tlight_distance = self._base_tlight_threshold
        affected_by_tlight, _, trafficCodeReturn = self._affected_by_traffic_light(lights_list, max_tlight_distance)
        ...
        if hazard_detected:
            ...
            return self.add_emergency_stop(control), trafficCodeReturn
        else:
            self._set_constant_velocity(self._target_speed)
            return control, trafficCodeReturn


    def run_transition(self, brake=False):
        """
        Function used to generate the transition function
        """
        if not self.is_constant_velocity_active:
            if self._world.get_snapshot().timestamp.elapsed_seconds - self._constant_velocity_stop_time > self._restart_time:
                self.restart_constant_velocity()
        ...
        # trafficCodeReturn 1 means the traffic light is observed red
        max_tlight_distance = self._base_tlight_threshold
        _, _, trafficCodeReturn = self._affected_by_traffic_light(lights_list, max_tlight_distance)

        # The longitudinal PID is overwritten by the constant velocity but it is
        # still useful to apply it so that the vehicle isn't moving with static wheels
        control = self._local_planner.run_step()

        # Force the provided action.
        if brake:
