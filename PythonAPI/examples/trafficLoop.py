...
# Modified file used for the experiment described in the bachelor thesis by Enrico Bonsu.
#
# Original by authors: German Ros (german.ros@intel.com)
...
def game_loop(args, client, traffic_manager, scenario=0):
    """
    Client and window initializer for CARLA
    """
...
def game_loop2(traffic_manager, world, scenario=0):
    """
    Initialize vehicle agent in CARLA.
    """
    try:
...
def tick_irl_agent(world, agent, destination, traffic_manager, brake=False):
    try:
        ...
        control, trafficLightCode = agent.run_transition(brake=brake)

        control.manual_gear_shift = False
        world.player.apply_control(control)
        return agent.done(), trafficLightCode
...
def tick_generate_transitions(world, agent, destination, traffic_manager, brake=False):
    """
    Function used to perform an action when generating the Transition function.
    """
    try:
        if agent.done():
            return True, 0
...
def tick_action(world, agent, destination, traffic_manager, features=None, scenario=0, client=None):
    """
    Function used to let an expert agent perform its actions.
    """
    try:
        trajectoryDone = False
        if agent.done():
            # Set spawnpoint when the agent has reached its destination
            trajectoryDone = True
            ...
            # Let the client teleport the agent
            if client:
        ...
        control, trafficLightCode = agent.run_step()
        control.manual_gear_shift = False
        world.player.apply_control(control)
        return trafficLightCode, trajectoryDone, 1 if control.throttle == 0 else 0
